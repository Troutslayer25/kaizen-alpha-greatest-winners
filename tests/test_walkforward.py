...
def test_expanding_splits_purge_and_disjoint_tests():
    ...
    # test periods across folds are non-overlapping
...
def test_ticker_disjoint_probe():
    tickers = np.repeat(np.arange(10), 5)   # 10 tickers, 5 obs each
    seen_test = set()
    for train_idx, test_idx in ticker_disjoint_folds(tickers, n_splits=5):
        ...
        assert train_tk.isdisjoint(test_tk)   # no ticker in both sides
        seen_test |= test_tk
    assert seen_test == set(range(10))         # every ticker tested once
