...
# APPENDIX F — VERSION 10.0 CHANGE LOG

V10.0 adds robustness hardening from a further methodology review. All earlier appendices retained.

**Change BB — Adversarial synthetic-oracle regime.** The clean oracle (pristine engineered V-bottoms) is necessary for scoring detection *localization* and proving leakage-freedom — you need exact ground truth, which requires clean paths. But a clean oracle never stress-tests the pipeline against messy reality. `generate_panel(adversarial=True)` (gws/synthetic/generate.py) injects flash-crash artifacts, erratic volume spikes, false-breakout (shakeout) bottoms, and obscured signal, re-deriving ground-truth troughs from the corrupted close so the panel stays scoreable. Tests assert the detector still locates planted moves and that A3 still isolates the planted signal from the null under chaos. Auditor 2 verifies robustness was demonstrated on the adversarial regime, not only the clean one.

**Change CC — Early-drama clustering dimensions.** Over-the-whole-move smoothness cannot distinguish a violent early shakeout from a calm grind of identical overall smoothness — conflating two behavioral regimes. `MoveMFE` now records `early_smoothness` (path efficiency of the first third) and `drawdown_timing` (location of the deepest from-peak drawdown, 0=early). These describe path SHAPE, not drawdown DEPTH, so they are uncensored by the trailing-stop parameter (the reason raw drawdown was kept comparative-only). They are comparative clustering inputs (run alongside the magnitude/duration/smoothness baseline), and `segment_by_early_drama()` splits the continuous-spectrum fallback into shakeout vs. immediate-ascender subsets. The detector stays neutral — these organize already-detected moves; they are not detection gates. *Note: a raw MAE/ATR "shakeout ratio" was considered and rejected — early MAE depth is itself bounded by the trailing stop, so it would re-import the censoring; path-shape metrics avoid that.*

**Change DD — A3 collinearity/VIF diagnostic.** A feature and the market-context regime can be near-duplicates (e.g. an individual price-to-200d-MA feature and a breadth regime built from the same MAs aggregated). `gws/phase_a3/collinearity.py` reports per-feature VIF, high-correlation pairs, and feature-vs-regime collinearity. It is a *diagnostic*, not an automatic transform (silent residualization would bake in a choice; the factor/industry neutralization step already removes "this feature is just the trend"). Design preference: build the regime score from non-price factors (credit/macro) where possible, orthogonal by construction. Auditor 3 reviews the diagnostic; for tree models, collinearity is noted as an attribution hazard, not an OOS-AUC inflation (trees are robust to it for prediction).
