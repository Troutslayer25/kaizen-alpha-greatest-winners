...
| v3 | 2026-06-21 | **Added trend-following data-capture requirements** (comprehensiveness, not bias — see `research/trend_following_foundations.md`): (a) **sector/industry classification + sector return series** from Norgate (potentially deep-history-capable; effective depth confirmed by the completeness audit); (b) **earnings report dates** as event markers (FMP; modern-window only; joins the fundamental branch, NULL in deep history); (c) **free float** (FMP; modern-window only; recorded as a feature, never a gate). All are recorded as features/markers, never eligibility gates. Pre-implementation; nothing run. |
